...
<body class="bg-gray-50 text-gray-800 antialiased">
            ...
            <div class="flex justify-between h-16">
                ...
                <div class="flex items-center gap-3">
                    ...
                    <a href="{{ route('products.create') }}"
                        ...
                        <span class="mr-2 bg-blue-200 text-blue-700 rounded-full w-5 h-5 flex items-center justify-center text-xs">
                            +
                        </span>
                        Nouveau Produit
                    </a>
    ...
    <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            ...
            <div class="group bg-white rounded-2xl border border-gray-200 shadow-sm hover:shadow-xl transition-all duration-300 flex flex-col overflow-hidden">

                <div class="relative h-48 bg-gray-100 overflow-hidden">
                    @if($product->image)
                    <img src="{{ asset('images/' . $product->image) }}" alt="{{ $product->name }}" class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500">
                    @else
                    ...
                    <div class="absolute top-3 left-3 flex flex-col gap-2">
                        ...
                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-bold bg-yellow-400 text-yellow-900 shadow-sm">
                            ★ Premium
                        </span>
                        @endif
                    </div>
                ...
                <div class="p-5 flex-1 flex flex-col">
                    ...
                    <div class="flex items-center justify-between mb-4 pb-4 border-b border-gray-100">
                        <div class="flex flex-col">
                            ...
                            <span class="text-lg font-bold text-blue-600">{{ $product->price }} <span class="text-sm font-normal text-gray-500">Tokens</span></span>
                        </div>
                        <div class="flex flex-col text-right">
                            <span class="text-xs text-gray-400 uppercase font-semibold">Stock</span>
                            <span class="text-sm font-bold {{ $product->stock < 5 ? 'text-red-500' : 'text-gray-700' }}">
                    ...
                    <div class="flex gap-2">
                        <a href="{{ route('products.edit', $product->id) }}" class="flex-1 bg-white border border-gray-300 text-gray-700 text-sm font-medium py-2 px-3 rounded-lg hover:bg-gray-50 hover:text-blue-600 transition text-center flex items-center justify-center gap-2">
                        ...
                        <form action="{{ route('products.destroy', $product->id) }}" method="POST" onsubmit="return confirm('Êtes-vous sûr de vouloir supprimer ce produit ?');">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="bg-red-50 text-red-600 border border-red-100 p-2 rounded-lg hover:bg-red-100 hover:text-red-700 transition" title="Supprimer">
            ...
            @empty
            <div class="col-span-full flex flex-col items-center justify-center py-12 text-center bg-white rounded-2xl border border-gray-200 border-dashed">
                ...
                <a href="{{ route('products.create') }}" class="text-blue-600 hover:underline font-medium">Créer le premier produit</a>
            </div>
            @endforelse
